...
relay_overflow = InelsHacsSwitchAlert(key="overflow", message="Relay overflow in %s of %d")


@dataclass
class InelsHacsSwitchType:
    """Inels switch property description"""

    name: str = "Relay"
    icon: str = ICON_SWITCH
    overflow: str | None = None
    alerts: list[InelsHacsSwitchAlert] | None = None


INELS_HACS_SWITCH_TYPES: dict[str, InelsHacsSwitchType] = {
    "bit": InelsHacsSwitchType(),
    "simple_relay": InelsHacsSwitchType(),
    "relay": InelsHacsSwitchType(alerts=[relay_overflow]),
}
...
async def async_setup_entry(
    ...
    items = INELS_HACS_SWITCH_TYPES.items()
                ...
                if len(device.state.__dict__[key]) == 1:
                    entities.append(
                        InelsHacsBusSwitch(
                            ...
                            key=key,
                            index=0,
                            description=InelsHacsSwitchEntityDescription(
                                key=key,
                                name=type_dict.name,
                                icon=type_dict.icon,
                    ...
                    for k in range(len(device.state.__dict__[key])):
                        ...
                        if device.inels_hacs_type == 'BITS':
...
@dataclass
class InelsHacsSwitchEntityDescription(SwitchHacsEntityDescription):
...
class InelsHacsBusSwitch(InelsHacsBaseEntity, SwitchEntity):
    """The platform class required by Home Assistant, bus version."""

    entity_description: InelsHacsSwitchEntityDescription
