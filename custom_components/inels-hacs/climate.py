...
INELS_HACS_CLIMATE_TYPES: dict[str, InelsHacsClimateType] = {
    "thermovalve": InelsHacsClimateType(
        name="Thermovalve",
        features= [ClimateEntityFeature.TARGET_TEMPERATURE],
        hvac_modes=[HVACMode.OFF, HVACMode.HEAT],
    ),
    "climate_controller": InelsHacsClimateType(
...
async def async_setup_entry(
    ...
    items = INELS_HACS_CLIMATE_TYPES.items()
            ...
            if hasattr(device.state, key):
                entities.append(
                    InelsHacsClimate(
                        ...
                        key=key,
                        index=-1,
                        description=InelsHacsClimateDescription(
                            key=key,
                            name=type_dict.name,
...
@dataclass
class InelsHacsClimateDescription(ClimateEntityDescription):
...
class InelsHacsClimate(InelsHacsBaseEntity, ClimateEntity):
    """Inels Climate entity for HA."""

    _attr_temperature_unit: str = UnitOfTemperature.CELSIUS

    entity_description: InelsHacsClimateDescription

    def __init__(
        self, device: Device, key: str, index: int, description: InelsHacsClimateDescription
    ) -> None:
