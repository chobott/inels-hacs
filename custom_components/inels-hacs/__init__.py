...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    inels_hacs_data: dict[str, Any] = {
        BROKER_CONFIG: entry.data,
    }

    mqtt: InelsMqtt = await hass.async_add_executor_job(
        InelsMqtt, inels_hacs_data[BROKER_CONFIG]
    )

    inels_hacs_data[BROKER] = mqtt

    entry.async_on_unload(entry.add_update_listener(async_reload_entry))

    if isinstance(  # None -> no error, int -> error code
        await hass.async_add_executor_job(inels_hacs_data[BROKER].test_connection), int
    ):
        return False

    if hass.data.get(DOMAIN) is None:
        hass.data.setdefault(DOMAIN, {})[entry.entry_id] = inels_hacs_data

    try:
        i_disc = InelsDiscovery(inels_hacs_data[BROKER])
        await hass.async_add_executor_job(i_disc.discovery)

        inels_hacs_data[DEVICES] = i_disc.devices
    ...
    inels_hacs_data[OLD_ENTITIES] = old_entries

    hass.data[DOMAIN][entry.entry_id] = inels_hacs_data
