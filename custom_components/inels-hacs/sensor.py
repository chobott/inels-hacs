...
INELS_HACS_SENSOR_TYPES: dict[str, InelsHacsSensorType] = {
    "temp": InelsHacsSensorType(
        name="Temperature sensor",
        icon=ICON_TEMPERATURE,
        unit=UnitOfTemperature.CELSIUS,
        device_class=SensorDeviceClass.TEMPERATURE,
    ),
    TEMP_IN: InelsHacsSensorType(
        name="Internal temperature sensor",
        icon=ICON_TEMPERATURE,
        unit=UnitOfTemperature.CELSIUS,
        device_class=SensorDeviceClass.TEMPERATURE,
    ),
    TEMP_OUT: InelsHacsSensorType(
        name="External temperature sensor",
        icon=ICON_TEMPERATURE,
        unit=UnitOfTemperature.CELSIUS,
        device_class=SensorDeviceClass.TEMPERATURE,
    ),
    "light_in": InelsHacsSensorType(
        name="Light intensity",
        icon=ICON_LIGHT_IN,
        unit=LIGHT_LUX,
        device_class=SensorDeviceClass.ILLUMINANCE,
    ),
    "ain": InelsHacsSensorType(
        name="Analog temperature sensor",
        icon=ICON_TEMPERATURE,
        unit=UnitOfTemperature.CELSIUS,
        device_class=SensorDeviceClass.TEMPERATURE,
    ),
    "humidity": InelsHacsSensorType(
        name="Humidity",
        icon=ICON_HUMIDITY,
        unit=PERCENTAGE,
        device_class=SensorDeviceClass.HUMIDITY,
    ),
    "dewpoint": InelsHacsSensorType(
        name="Dew point",
        icon=ICON_DEW_POINT,
        unit=UnitOfTemperature.CELSIUS,
        device_class=SensorDeviceClass.TEMPERATURE,
    ),
    "temps": InelsHacsSensorType(
        name="Temperature sensor",
        icon=ICON_TEMPERATURE,
        unit=UnitOfTemperature.CELSIUS,
        indexed=True,
...
@dataclass
class InelsHacsSensorDescriptionMixin:
    """Mixin keys."""


@dataclass
class InelsHacsSensorDescription(SensorEntityDescription, InelsHacsSensorDescriptionMixin):
...
async def async_setup_entry(
    ...
    items = INELS_HACS_SENSOR_TYPES.items()
                    ...
                    for k in range(len(device.state.__dict__[key])):
                        entities.append(
                            InelsHacsSensor(
                                ...
                                key=key,
                                index=k,
                                description=InelsHacsSensorDescription(
                                    key=f"{key}{k}",
...
class InelsHacsSensor(InelsHacsBaseEntity, SensorEntity):
    """Platform class for Home assistant, bus version."""

    entity_description: InelsHacsSensorDescription
